...
static void guest_code(uint64_t base_gva)
{
	...
	for (i = 0; i < config->test_num_pages; i++) {
		uint64_t *src = (uint64_t *)(base_gva + i * config->page_size);
			     ...
			     bool private)
{
	 ...
	 */
	TEST_ASSERT_EQ(pthread_create(&slot_worker, NULL, delete_slot_worker, &data), 0);
	...
	for (;;) {
		...
		if (!slot_recreated) {
			...
			TEST_ASSERT_EQ(pthread_join(slot_worker, NULL), 0);
			slot_recreated = true;
	...
	TEST_ASSERT(range.size == expected_left,
		    "Completed with %llu bytes left, expected %llu",
		    (unsigned long long)range.size,
		    (unsigned long long)expected_left);
...
{
	uint64_t gpa, gva, alignment, guest_page_size, host_page_size;
	uint64_t backing_src_pagesz, mem_page_size;
