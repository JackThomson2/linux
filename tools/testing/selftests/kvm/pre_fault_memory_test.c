...
static void __test_pre_fault_memory(enum vm_guest_mode mode, void *arg)
{
	...
	pre_fault_memory(vcpu, guest_test_phys_mem + TEST_BASE_SIZE, test_config.page_size * 2,  test_config.page_size);
	pre_fault_memory(vcpu, guest_test_phys_mem + test_config.test_size, test_config.page_size, test_config.page_size);
