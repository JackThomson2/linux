...
int kvm_async_pf_reject(struct kvm_vcpu *vcpu, gpa_t gpa)
{
	struct kvm_async_pf *apf;
	gfn_t gfn = gpa_to_gfn(gpa);
...
void kvm_clear_async_pf_completion_queue(struct kvm_vcpu *vcpu)
{
	...
	if (vcpu->async_pf.complete_eventfd) {
		eventfd_ctx_put(vcpu->async_pf.complete_eventfd);
		vcpu->async_pf.complete_eventfd = NULL;
	}
	if (vcpu->async_pf.pinned_page) {
		unpin_user_pages(&vcpu->async_pf.pinned_page, 1);
		vcpu->async_pf.pinned_page = NULL;
	}
...
static void kvm_apf_drain_complete_ring(struct kvm_vcpu *vcpu)
{
	...
	while (CIRC_CNT(head, tail, KVM_APF_RING_SIZE)) {
		...
		smp_store_release(&ring->tail, (tail + 1) % KVM_APF_RING_SIZE);
		tail = ring->tail;
				    ...
				    int sync, void *key)
{
	...
	if (flags & EPOLLIN) {
		...
		eventfd_ctx_do_read(vcpu->async_pf.complete_eventfd, &cnt);
