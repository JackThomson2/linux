...
static struct kvm_async_pf *async_pf_find_work_item_from_gfn(struct kvm_vcpu *vcpu, gfn_t gfn)
{
	struct kvm_async_pf *apf, *tmp;
...
int kvm_async_pf_userfault_ready(struct kvm_vcpu *vcpu, gpa_t gpa)
{
	bool first;
	struct kvm_async_pf *apf;
	...
	if (apf->arch.state == KVM_APF_UF_COMPLETED) {
		spin_unlock(&vcpu->async_pf.lock);
		return -EFAULT;
	}
